...
class WeightUpdate implements ShouldQueue
{
     ...
     */
    protected $booking;
    protected $actualArrivalDispatchWeight;
    protected $arrivalDispatchWeight;
    protected $booked_packet_vol_weight_cal;
    protected $booked_packet_weight;
    protected $request;
    protected $user;

    public function __construct($bookingIds,$actualArrivalDispatchWeight,$arrivalDispatchWeight, $booked_packet_vol_weight_cal, $booked_packet_weight, $request, $userId)
    {
        $this->queue = 'batch_weight_update';
        $this->booking =  $bookingIds;
        $this->actualArrivalDispatchWeight = $actualArrivalDispatchWeight;
        $this->arrivalDispatchWeight = $arrivalDispatchWeight;
        $this->booked_packet_vol_weight_cal =  $booked_packet_vol_weight_cal;
        $this->booked_packet_weight = $booked_packet_weight;
    ...
    public function handle()
    {
        foreach ($this->booking as $key => $id) {
            ecom_booking::where('id', $id)->update([
                'arival_dispatch_weight' => $this->arrivalDispatchWeight[$key],
                'booked_packet_vol_weight_cal' => $this->booked_packet_vol_weight_cal[$key],
                'booked_packet_weight' => $this->booked_packet_weight[$key],
            ]);
            ...
            if(!empty($teriff)) {
                ...
                $update =  ecom_booking::where('id',$id)->update($rateArray);
            }
            ecom_activity_remark::addNewremark('batch_weight_update', $id, $this->user, 0, 'Batch update - update arival_dispatch_weight from '.$this->actualArrivalDispatchWeight[$key].' to '.$this->arrivalDispatchWeight[$key].'');
        }
