...
class OmsArrivalSync extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'default:oms-data-arrival';
    ...
    public function handle()
    {
        ...
        $cron_job->status = 1;
        $cron_job->cron_name = 'default:oms-data-arrival';
        $cron_job->created_at =  date('Y-m-d H:i:s');


        $result = ecom_booking::selectRaw('MIN(cn_short) as min_value, MAX(cn_short) as max_value')->first();
        $start_date = date('Y-m-d', strtotime('-3 month'));
        $current_date = date('Y-m-d');

        $CN_LIST = array();
        $maxSystemTime = date('Y-m-d H:i:s', strtotime('-2 hour'));
        ...
        for ($current = strtotime($start_date); $current <= strtotime($current_date); $current = strtotime('+3 month', $current)) {
            ...
            $cnWithPrefix = ecom_booking::whereBetween('cn_short', [$result->min_value, $result->max_value])
                ...
                ->whereIn('booked_packet_status',[2, 4, 5, 6, 8, 9,10, 14, 16, 17,20])
                ->where('is_deleted', 0)
                ->where('is_cancel', 0)
            ...
            if(count($ourArrivalCnNumber) > 0) {
                $maxSystemTime = date('Y-m-d H:i:s', strtotime(max($ourArrivalSystemTime).'-15 minutes'));
            }
