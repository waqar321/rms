...
class TrackiFyCronByArrivalHourly extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'default:trackify_by_arrival_hourly';
    ...
    public function handle()
    {
        ...
        $cron_job->status = 1;
        $cron_job->cron_name = 'default:trackify_by_arrival_hourly';
        ...
        for ($current = strtotime($start_date); $current <= strtotime($current_date); $current = strtotime('+1 day', $current)) {
            $current_formatted = date('Y-m-d H:i:s', $current);
            $end_date = $current_formatted;

            $error_log = array();
            $bookedPacketList = ecom_arrival_oms::GetDataForTrackifyHourly($current_formatted,$end_date,null);
