...
class OmsDispatchSync extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'default:oms-data-dispatch';
    ...
    public function handle()
    {
        ...
        $cron_job->status = 1;
        $cron_job->cron_name = 'default:oms-data-dispatch';
        ...
        for ($current = strtotime($start_date); $current <= strtotime($current_date); $current = strtotime('+3 month', $current)) {
            ...
            if(count($ourDispatchCnNumber) > 0) {
                $maxSystemTime = date('Y-m-d H:i:s', strtotime(max($ourDispatchSystemTime).'-2 hour'));
            }
