...
class updateWeightBiWeekly extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'default:update_weight_biweekly';
     ...
     * @var string
     */
    protected $description = 'COD Update Booking Weight Bi Weekly';
    ...
    public function handle()
    {
        ...
        $cron_job->status = 1;
        $cron_job->cron_name = 'default:update_weight_biweekly';
        $cron_job->created_at =  date('Y-m-d H:i:s');

        $days_start_update = 15;
        $days_end_update = 8;
        $start_date = date('Y-m-d', strtotime(date('Y-m-d') . ' -' . $days_start_update . ' days'));
        $end_date =  date('Y-m-d', strtotime(date('Y-m-d') . ' -' . $days_end_update . ' days'));
            ...
            if(count($bookedPacketList) > 0){
                ...
                if (count($weight_dataList) > 0) {
                    $ecom_booking = ecom_booking::whereIn('booked_packet_cn', $bookedPacketList)
                        ...
                        //whereIn('booked_packet_cn',["KI751163670","KI751355164","KI751467762","KI751467775","KI751924892","KI752211467"])
