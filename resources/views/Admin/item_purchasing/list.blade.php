                                    ...
                                    <button class="btn btn-info btn-sm uppercase mr-1" type="button"> Total Sale:
                                        @if($readyToLoad)
                                            @if($searchByItem_id != null || $searchByCategory_id != null || $searchByItemName != null)
                                                {{ $total_item_sale }}
                                            @else
                                                {{ $Purchases->getCollection()->sum('total_amount') }}
                                            @endif
                                        @endif
                                    </button>
                            ...
                            <tbody>
                                @if($readyToLoad)
                                    @forelse($Purchases as $Purchase)
                                        ...
                                        <tr>
                                            <td>
                                                <input type="checkbox" wire:model="selectedRows.{{ $Purchase->id }}">
                                            </td>
                                            <td>{{ $Purchase->id }}</td>
                                            <td>
                                                <div class="d-flex flex-wrap gap-2">
                                                    @foreach($Purchase->purchaseItems as $PurchaseItem)
                                                        ...
                                                        @if($searchByItem_id != null)
                                                            @if($PurchaseItem->item_id == $searchByItem_id)
                                                                <div class="text-center zoom-wrapper">
                                                                    <div class="zoom-content">
                                                                        <img src="{{ asset('storage/' . $PurchaseItem->item->image_path) }}"
                                                                            alt="{{ $PurchaseItem->item->name }}"
                                                                            class="img-thumbnail"
                                                                            style="height: 60px; object-fit: cover;">
                                                                        <!-- <small class="d-block mt-1">{{ $PurchaseItem->item->name }} : {{ $PurchaseItem->item_qty }}</small> -->
                                                                        <small class="d-block mt-1">{{ $PurchaseItem->item->name }} : {{ $PurchaseItem->item_qty }} </small>
                                                                    </div>
                                                                </div>
                                                            @endif
                                                        @elseif($searchByCategory_id != null)
                                                            @if($PurchaseItem->item->category_id == $searchByCategory_id)
                                                        ...
                                                        @elseif($searchByItemName != null)
                                                            @if(in_array($PurchaseItem->item->id, $this->item_ids))
                                                            ...
                                                            <div class="text-center zoom-wrapper">
                                                                <div class="zoom-content">
                                                                    <img src="{{ asset('storage/' . $PurchaseItem->item->image_path) }}"
                                                                        ...
                                                                        style="height: 60px; object-fit: cover;">
                                                                    <!-- <small class="d-block mt-1">{{ $PurchaseItem->item->name }} : {{ $PurchaseItem->item_qty }}</small> -->
                                                                    <small class="d-block mt-1">{{ $PurchaseItem->item->name }} : {{ $PurchaseItem->item_qty }} : {{ $searchByItem_id }} </small>
                                                                </div>
                                                            </div>
                                                        @endif
                                            ...
                                            <td>{{ $Purchase->entry_person->username }}</td>
                                            <td>{{ $Purchase->created_at }}</td>
                                            <!-- <td>
                                                @if($Purchase->is_active == 1)
                                                    <span class="fa fa-toggle-on toggle-icon" wire:click="updateStatus({{ $Purchase->id }}, 0)"></span>
                                                @else
                                                    <span class="fa fa-toggle-off toggle-icon" wire:click="updateStatus({{ $Purchase->id }}, 1)"></span>
                                                @endif
                                            </td> -->
                                            <td>
                                                <button wire:click="RowReceiptPrintOut({{ $Purchase->id }})" class="btn btn-success w-100">Print Slip</button>

                                                <!-- <a data-screen-pos-id="23" wire:click="EditData({{ $Purchase->id }})" class="btn btn-primary">Edit</a> -->
                                                <button data-screen-pos-id="24" onclick="confirmDelete('{{ $Purchase->id }}')" class="btn btn-danger">Delete</button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
