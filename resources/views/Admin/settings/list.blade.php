    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            ...
            <div class="x_content">
                            ...
                            <tbody>
                                ...
                                @if($readyToLoad)
                                    @forelse($Settings as $setting)
                                        <tr>
                                            <!-- <td>
                                                <input type="checkbox" wire:model="selectedRows.{{ $setting->id }}">
                                            </td> -->
                                            <td>{{ $setting->id }}</td>
                                            <td>{{ $setting->name  }}</td>                                            
                                            <td>
                                                @if($setting->is_active == 1)
                                                    <span class="fa fa-toggle-on toggle-icon" wire:click="updateStatus({{ $setting->id }}, 0)"></span>
                                                @else
                                                    <span class="fa fa-toggle-off toggle-icon" wire:click="updateStatus({{ $setting->id }}, 1)"></span>
                                                @endif
                                            </td>
                                            <td>
                                                <a data-screen-unit_type-id="23" wire:click="EditData({{ $setting->id }})" class="btn btn-primary">Edit</a>
                                                <button data-screen-unit_type-id="24" onclick="confirmDelete('{{ $setting->id }}')" class="btn btn-danger">Delete</button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="8" class="text-center"> <h2> No Setting Found!!! </h2></td>
                                        </tr>
                                    @endforelse
                                @endif 
                        ...
                        @include('Admin.partial.livewire.pagination', ['ModelListing' => $Settings, 'Model' => 'categoryListing'])       
