                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            ...
                            <form id="select_form" action="" novalidate="novalidate" data-parsley-validate
                                ...
                                <div class="col-md-4 col-sm-6 col-xs-12" style="display:none" id="shipper_div">
                                    <div class="form-group">
                ...
                <div class="modal-content">
                    ...
                    <form id="payment-form" action="" novalidate="novalidate" data-parsley-validate class="form-horizontal form-label-left" method="post">
                        <div class="modal-body form-horizontal" style="max-height: 400px;overflow: auto;">
                            ...
                            <div class="form-group">
                                <label class="col-md-4 control-label">Payee Selection</label>
                                <div class="col-md-8">
                                    <select data-msg-required="This is required" name="payee_id" id="payee_id" class="form-control select2">
                            ...
                            <div class="form-group">
                                <label class="col-md-4 control-label">Account#<span class="danger"></span></label>
                                <div class="col-md-8">
                            ...
                            <div class="form-group">
                                <label class="col-md-4 control-label">IBAN No#<span class="danger"></span></label>
                                <div class="col-md-8">
                                    <input type="text" id="iban_no" class="form-control" value="" readonly>
                                    <span class="error-container danger w-100"></span>
                                </div>
                            </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <input type="hidden" name="shipper_id_process" id="shipper_id_process">
    ...
    @section('scripts')
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="col-md-4" style="border-left: 1px solid #e7ecf1;">
                                            ...
                                            <table class="table" id="packing-materials">
                                                <tbody></tbody>
                                                <input type="hidden" id="material-rate" value="0">
                                            </table>
                                            <hr>
                                            <table class="table">
                                                <tbody>
        ...
        `;
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        if (data && data.status == 1) {
                                            ...
                                            $('#push_data').html(push_data);

                                            populateClientDetails(data.data.clientInformation);
                                            populateBankInformation(data.data.clientBankInformation);
                                            ...
                                            if(data.data.notPayablePackets.length > 0){
                                                populateNotPayableList(data.data.notPayablePackets,data.data.clientInformation);
                                            }

                                            if(data.data.payablePackets.length > 0){
                                                populatePayableList(data.data.payablePackets,data.data.clientInformation);
                                            }
                    ...
                    $(keys).each(function (index, element) {
                        if(element != 'merchant_id'){
                            ...
                            if(element == 'city_name'){
                                $('#'+element).text(values[index]+', Pakistan');
                            }
                ...
                function populateClientDetails(data){
                    $('#shipper_id_process').val(data.merchant_id);
                    populateDetails(data);
                    $('.modal-footer #city_id').val(data.city_id);
                }

                function populateBankInformation(data){

                    $.each(data, function(index, item) {
                        $('#payee_id').append($('<option>', {
                            value: item.id,
                            text: item.bank_ac_title,
                            selected: index === 0,
                            'data-account-number': item.bank_ac_no,
                            'data-iban': item.iban_no
                        }));
                    });

                    $('#invoice_cheque_holder_name').val(data[0].bank_ac_title);
                    $('#account_no').val(data[0].bank_ac_no);
                    $('#iban_no').val(data[0].iban_no);
                }

                $('#payee_id').on('change', function() {
                    var selectedOption = $(this).find(':selected');
                    var bankAcTitle = selectedOption.text();
                    var bankAcNo = selectedOption.data('account-number');
                    var ibanNo = selectedOption.data('iban');
                    
                    $('#invoice_cheque_holder_name').val(bankAcTitle);
                    $('#account_no').val(bankAcNo);
                    $('#iban_no').val(ibanNo);
                });
                ...
                function populateNotPayableList(data,clientData){
                    ...
                    data.forEach(item => {
                        ...
                        if (item.merchant_id == 595481 || clientData.is_settlement==0) {
                            item.packet_charges = 0;
                        }
                        ...
                        TotalBookedPacketWeight += Math.max(item.booked_packet_weight,item.arival_dispatch_weight)/1000;
                        ...
                        if(item.booked_packet_status==12) status = 'Deliverd'; else status = 'Under Process';
                        var row = [
                            ...
                            item.city_name,
                            zone,
                            Math.max(item.booked_packet_weight,item.arival_dispatch_weight)/1000,
                ...
                function populatePayableList(data,clientData){

                    $('#readyForChequeTable').DataTable().clear().draw();

                    var table =  $('#readyForChequeTable').DataTable();
                ...
                function computeSummaryCharges(TotalCODAmount,TotalReturnCODAmount,TotalCharges,Totalfuelsurge,TotalGST){
                    ...
                    $('#gross_invoice_1').text("-"+gross_charges.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                    total_payable = amount_payable - gross_charges;
                    cheqpayable = total_payable - $('#material-rate').val();
                    $('#total_payable').text(total_payable.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                    $('#cheque_payable').text(cheqpayable.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                    $('#invoice_cheque_amount').val(Math.ceil(cheqpayable));
                    $('.modal-footer #total_amount').val(TotalCODAmount);
                    $('.modal-footer #total_charges').val(TotalCharges);
                    $('.modal-footer #total_gst').val(TotalGST);
                    $('.modal-footer #total_return').val(TotalReturnCODAmount);
                ...
                $('body').delegate('#process_pay', 'click', function(e) {
                ...
                function populatePackingMaterials(data){
                    ...
                    data.forEach(item => {
                        ...
                        $('#material-rate').val(total_material);
