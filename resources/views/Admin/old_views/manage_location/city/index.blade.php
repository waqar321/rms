...
@section('title') City Listing @endsection
...
{{--                    </form>--}}
                    ...
                    <div class="x_content">
                        <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="55">
                            <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        buttons: [
            {
                text: '<i class="la la-cogs"></i> Add City',
                className: 'btn btn-primary btn-sm add',
                action: function (e, dt, node, config) {
                    window.location = '<?php echo url_secure('manage_location/city/add') ?>';
                },
                init: function (dt, node, config) {
                    // Add custom data attributes to the button
                    $(node).attr('data-screen-permission-id', "52");
                }
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('manage_location/city') ?>',
            data: function (d) {
                d.ajax=1;
            },
            headers: headers,
        ...
        columns: [
            ...
            {data: 'id', name: 'ecom_city.id', class: 'align-middle id',text:'Sys ID#' ,download:true},
            {data: 'country_name', name: 'con.country_name', class: 'align-middle country_name',text:'Country Name',download:true},
            {data: 'state_name', name: 'st.state_name', class: 'align-middle state_name',text:'State Name',download:true},
            {data: 'city_name', name: 'ecom_city.city_name', class: 'align-middle city_name',text:'City Name',download:true},
            {data: 'city_abbr', name: 'ecom_city.city_abbr', class: 'align-middle city_abbr',text:'City Abbr',download:true},
            {data: 'city_email', name: 'ecom_city.city_email', class: 'align-middle city_email',text:'City Email',download:true},
            {data: 'created_at', name: 'ecom_city.created_at', class: 'align-middle created_at',text:'Date Created',download:true},
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('manage_location/city/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
