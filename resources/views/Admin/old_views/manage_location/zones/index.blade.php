...
@section('title') Zones Listing @endsection
...
{{--                    </form>--}}
                    ...
                    <div class="x_content">
                        <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="50">
                            <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        buttons: [
            {
                text: '<i class="la la-cogs"></i> Add Zone',
                className: 'btn btn-primary btn-sm add',
                action: function (e, dt, node, config) {
                    window.location = '<?php echo url_secure('manage_location/zone/add') ?>';
                },
                init: function (dt, node, config) {
                    // Add custom data attributes to the button
                    $(node).attr('data-screen-permission-id', "48");
                }
            },
        ],
        select: {
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('manage_location/zone') ?>',
            data: function (d) {
                d.ajax=1;
            },
            headers: headers,
        ...
        columns: [
            ...
            {data: 'id', name: 'ecom_zone.id', class: 'align-middle id',text:'Sys ID#' ,download:true},
            {data: 'country_name', name: 'con.country_name', class: 'align-middle country_name',text:'Country Name',download:true},
            {data: 'zone_name', name: 'ecom_zone.zone_name', class: 'align-middle zone_name',text:'Zone Name',download:true},
            {data: 'created_at', name: 'ecom_zone.created_at', class: 'align-middle created_at',text:'Date Created',download:true},
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('manage_location/zone/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
