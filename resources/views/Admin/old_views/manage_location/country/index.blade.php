...
@section('title') Country Listing @endsection
...
{{--                                <button type="submit" class="btn col-md-3 col-sm-3 col-xs-12 export-button">Export</button>--}}
{{--                            </div>--}}
{{--                        </div>--}}
{{--                    </form>--}}

                    <div class="x_content">
                        <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="40">
                            <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        buttons: [
            {{--{--}}
            {{--    text: '<i class="la la-cogs"></i> Add Country',--}}
            {{--    className: 'btn btn-primary btn-sm add',--}}
            {{--    action: function (e, dt, node, config) {--}}
            {{--        window.location = '<?php echo url_secure('manage_location/country/add') ?>';--}}
            {{--    },--}}
            {{--    init: function (dt, node, config) {--}}
            {{--        // Add custom data attributes to the button--}}
            {{--        $(node).attr('data-screen-permission-id', "37");--}}
            {{--    }--}}
            {{--},--}}
        ],
        select: {
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('manage_location/country') ?>',
            data: function (d) {
                d.ajax = 1;
            },
            headers: headers,
        ...
        columns: [
            ...
            {data: 'id', name: 'ecom_country.id', class: 'align-middle id',text:'Sys ID#' ,download:true},
            {data: 'country_name', name: 'ecom_country.country_name', class: 'align-middle country_name',text:'Country Name',download:true},
            {data: 'created_at', name: 'ecom_country.created_at', class: 'align-middle created_at',text:'Date Created',download:true},
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('manage_location/country/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
