...
@section('title') Account Type Listing @endsection
...
{{--                    </form>--}}
                    ...
                    <div class="x_content">
                        <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="16">
                            <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        buttons: [
            {
                text: '<i class="la la-cogs"></i> Add Account Type',
                className: 'btn btn-primary btn-sm add',
                action: function (e, dt, node, config) {
                    window.location = '<?php echo url_secure('manage/account_type/add') ?>';
                },
                init: function (dt, node, config) {
                    // Add custom data attributes to the button
                    $(node).attr('data-screen-permission-id', "13");
                }
            },
        ],
        select: {
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('manage/account_type/index') ?>',
            data: function (d) {
                d.ajax = 1;
            },
            headers: headers,
        ...
        columns: [
            ...
            {data: 'id', name: 'ecom_account_type.id', class: 'align-middle id',text:'Sys ID#' ,download:true},
            {data: 'account_type_name', name: 'ecom_account_type.account_type_name', class: 'align-middle account_type_name',text:'AccountType Name',download:true},
            {data: 'created_at', name: 'ecom_account_type.created_at', class: 'align-middle created_at',text:'Date Created',download:true},
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('manage/account_type/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
