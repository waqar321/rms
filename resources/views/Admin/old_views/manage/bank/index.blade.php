...
@section('title') Bank Listing @endsection
...
{{--                    </form>--}}
                    ...
                    <div class="x_content">
                        <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="25">
                            <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        buttons: [
            {
                text: '<i class="la la-cogs"></i> Add Bank',
                className: 'btn btn-primary btn-sm add',
                action: function (e, dt, node, config) {
                    window.location = '<?php echo url_secure('manage/bank/add') ?>';
                },
                init: function (dt, node, config) {
                    // Add custom data attributes to the button
                    $(node).attr('data-screen-permission-id', "22");
                }
            },
        ],
        select: {
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('manage/bank/index') ?>',
            data: function (d) {
                d.ajax = 1;
            },
            headers: headers
        ...
        columns: [
            ...
            {data: 'id', name: 'ecom_bank_list.id', class: 'align-middle id',text:'Sys ID#' ,download:true},
            {data: 'bank_name', name: 'ecom_bank_list.bank_name', class: 'align-middle bank_name',text:'Bank Name',download:true},
            {data: 'created_at', name: 'ecom_bank_list.created_at', class: 'align-middle created_at',text:'Date Created',download:true},
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('manage/bank/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
