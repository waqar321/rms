...
{{--                        <div class="col-md-12">--}}
{{--                            <div class="x_panel filter-bg">--}}
{{--                                <div class="x_title">--}}
{{--                                    <h2>Advance<small>Filter</small></h2>--}}
...
{{--                    </div>--}}
                    ...
                    <div class="x_content">
                        <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="11">
                            <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        buttons: [
            {
                text: '<i class="la la-cogs"></i> Add News',
                className: 'btn btn-primary btn-sm add',
                action: function (e, dt, node, config) {
                    window.location = '<?php echo url_secure('manage/news/add') ?>';
                },
                init: function (dt, node, config) {
                    // Add custom data attributes to the button
                    $(node).attr('data-screen-permission-id', "8");
                }
            },
        ],
        select: {
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('manage/news/index') ?>',
            data: function (d) {
                d.ajax = 1;
                d.status = $('#status').val();
            },
        ...
        columns: [
            ...
            {data: 'id', name: 'ecom_news.id', class: 'align-middle id',text:'Sys ID#' ,download:true},
            {data: 'news_title', name: 'ecom_news.news_title', class: 'align-middle news_title',text:'News title',download:true},
            {data: 'news_content', name: 'ecom_news.news_content', class: 'align-middle news_content limited',text:'News Description',download:true},
            {data: 'created_at', name: 'ecom_news.created_at', class: 'align-middle created_at',text:'Date Created',download:true},
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) 
        {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('manage/news/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
