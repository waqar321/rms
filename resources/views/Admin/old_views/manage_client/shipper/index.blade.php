...
@section('title') Shipper Listing @endsection
...
{{--                        </form>--}}
                        <div class="x_content">
                            <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="64">
                                <div class="col-md-4 col-sm-4 col-xs-12">
...
@section('scripts')
    ...
    <script>
        ...
        var table = $('#datatable').DataTable({
            ...
            buttons: [
                {
                    text: '<i class="la la-cogs"></i> Add Shipper',
                    className: 'btn btn-primary btn-sm add',
                    action: function (e, dt, node, config) {
                        window.location = '<?php echo url_secure('manage_client/shipper/add') ?>';
                    },
                    init: function (dt, node, config) {
                        // Add custom data attributes to the button
                        $(node).attr('data-screen-permission-id', "61");
                    }
                },
            ],
            select: {
            ...
            serverSide: true,
            ajax: {
                url: '<?php echo api_url('manage_client/shipper') ?>',
                data: function (d) {
                    d.ajax = 1;
                },
                headers: headers,
            ...
            columns: [
                ...
                },
                {
                    ...
                    class: 'align-middle phone limited',
                    text: 'Shipper Phone',
                    download: true
                },
                {
        ...
        jQuery(function ($) {
            ...
            $('#export-csv').submit(function (e) {
                ...
                if (!$('#export').val()) {
                    ...
                    $.ajax({
                        url: '<?php  echo api_url('manage_client/shipper/downloadCsv')  ?>', // Replace with your backend URL
                        type: 'GET',
                        data: {
