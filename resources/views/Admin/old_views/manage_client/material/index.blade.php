...
@section('title') Materials Listing @endsection
...
{{--                    </div>--}}
                    <div class="x_content">
                        <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="83">
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        buttons: [
            {
                text: '<i class="la la-cogs"></i> Add Packing Material',
                className: 'btn btn-primary btn-sm add',
                action: function (e, dt, node, config) {
                    window.location = '<?php echo url_secure('manage_client/material/add') ?>';
                },
                init: function (dt, node, config) {
                    // Add custom data attributes to the button
                    $(node).attr('data-screen-permission-id', "80");
                }
            },
        ],
        select: {
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('manage_client/material') ?>',
            data: function (d) {
                d.ajax=1;
                d.status = $('#status').val();
            },
        ...
        columns: [
            ...
            {data: 'material_name', name: 'material_name', class: 'align-middle material_name',text:'Material Name',download:true},
            {data: 'created_at', name: 'created_at', class: 'align-middle created_at',text:'Created At',download:true},
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('manage_client/material/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
