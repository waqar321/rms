...
@section('title') Return Shipments @endsection
...
{{--                                <div class="col-md-4 col-sm-3 col-xs-12">--}}
{{--                                    <label class="control-label col-md-12 col-sm-12 col-xs-12">Return Reason</label>--}}
{{--                                    <div class="col-md-12 col-sm-12 col-xs-12">--}}
...
{{--                                </div>--}}
                        ...
                        <div class="x_content">
                            <form method="POST" enctype="multipart/form-data" action="#" id="export-csv" data-screen-permission-id="146">
                                <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('report_manager/return_shipments') ?>',
            data: function (d) {
                d.ajax=1;
                d.date = $('#date').val();
        ...
        columns: [
            ...
            {data: 'attempt_counts', name: 'ecom_bookings.is_active', class: 'align-middle attempt_counts',text:'Attempts',download:true, orderable: false, searchable: false},
            {data: 'date_return', name: 'ecom_bookings.date_return', class: 'align-middle date_return',text:'Return Date',download:true},
            {data: 'REASON', name: 'eao.REASON', class: 'align-middle REASON',text:'Return Reason',download:true},
            {data: 'return_address', name: 'ecom_bookings.return_address', class: 'align-middle return_address',text:'Return Address',download:true},
            {data: 'ret_city', name: 'return_city.city_name', as: 'ret_city', class: 'align-middle city_name',text:'Return City',download:true},
        ...
        initComplete: function () {
            ...
            this.api().columns().every(function (column_id) {
                ...
                if ($(header).is('.action') ||  $(header).is('.serial_number') ||  $(header).is('.select') ||  $(header).is('.status') || $(header).is('.attempt_counts')) {
                    $(td).appendTo($(search));
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('report_manager/return_shipments/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
                    ...
                    success: function (response) {
                        ...
                        a.href = url;
                        a.download = 'return_shipment_report_' + new Date().toISOString().slice(0, 10) + '.csv';
