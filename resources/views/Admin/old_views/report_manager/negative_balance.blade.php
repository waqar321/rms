...
@section('title') Negative Balance @endsection
...
<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo api_url('report_manager/negative_balance') ?>',
            data: function (d) {
                d.ajax=1;
                d.date = $('#date').val();
        ...
        columns: [
            ...
            {data: 'action', name: 'action', class: 'action',text:'action',download:false},
            ],
    ...
    jQuery(function ($) {
        ...
        $('#export-csv').submit(function (e) {
            ...
            if (!$('#export').val()) {
                ...
                $.ajax({
                    url: '<?php  echo api_url('report_manager/negative_balance/downloadCsv')  ?>', // Replace with your backend URL
                    type: 'GET',
                    data: {
                    ...
                    success: function (response) {
                        ...
                        a.href = url;
                        a.download = 'negative_balance_report_' + new Date().toISOString().slice(0, 10) + '.csv';
